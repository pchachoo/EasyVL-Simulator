...
int main(int argc, char *argv[])
{
    ...
    //	printf("dealing with Line %d \n", line_no);
        for (size_t i = 0; i < line.size();)
        {
            ...
            {
            	int number_begin = i;
         //   	printf("number begins at %d", i);
            	for (++i; i<line.size();i++){
            		if((line[i]>='0') && (line[i]<='9')){
          //  			printf("character at position %d is a number \n",i);
            			continue;
            		}
            		else{ //character is not a number
           // 			printf("character at position %d is not a number \n",i);
            			break;
            		}
            	}
          //  	printf("number ends at %d", i);
          //  	printf("number has length %d", i-number_begin);
