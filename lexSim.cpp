...
void writetofile(const std::vector<std::string> & modulenamevector,
...
unsigned UpdateCurrentValue(unsigned OrgNumber, unsigned NewNumber, int endIndex, int startIndex);
void CheckComponentExecStatus(struct componentstruct cd,unsigned pinindex,const std::vector<struct netstruct> & net,std::vector<struct netlist>&nl,unsigned *nlIndex,unsigned *clIndex);
int lex(std::ifstream &input_file,
		std::vector<std::string>&tokenvector);

void CreatenetList(std::vector<struct componentstruct> componentdetails,
			 std::vector<struct netstruct> unsortednet,std::vector<std::string> modulenamevector,
...
//struct trisvalue trisValue;
std ::vector<struct trisvalue> trisValue;//bonus
//unsigned trisvarcount = 0;//bonus
struct trisvalue temptv_vector;
unsigned no_of_transitions = 0;
std::ifstream readinput;//bonus1
...
int ComputeTrisValue(){
	...
	size = trisValue.size();
	if (size == 0) return 0;
	for(k = 0; k< trisValue.size()-1; k++)
	{
		//size = trisValue[k].wirevalueZ.size();
		startk = k;
		found = 0;
		count = 0;
		while((trisValue[k].wirename == trisValue[k+1].wirename)&&(k < (trisValue.size()-2)))
		{
			//std::cout<<"\ntrisValue.wirevalueZ.at(i) "<<trisValue.wirevalueZ.at(i)<<"i ="<<i<<std::endl;
			found = 1;
			if (trisValue[k].wirevalueZ == 0)
				count++;
			k++;
		}
		if ((trisValue[k+1].wirevalueZ == 0)&&(found))
		{
						count++;
		...
		{
			if (trisValue[i].wirevalueZ == 0)
			{
				wirevalueB = trisValue[i].wirevalueB;
				break;
			}
		}
		...
		for(j = 0, found = 0; j < currentValue.size(); j++)
		{
			if(currentValue[j].wirename.compare(trisValue[i].wirename) == 0)
			{
				found = 1;
				break;
			}
		...
		if(found)
			currentValue[j].wirevalue[0] = wirevalueB;
		//std::cout<<"\nName = " <<currentValue[j].wirename<<" "<<"value= "<<wirevalueB<<std::endl;
	}
	return 0;
}
			 ...
			 std::ofstream &netlist_file)
{
	...
	int nComp = 0;
	unsigned int i = 0, j = 0, k=0;//Nov
	for(i = 0; i < unsortednet.size();i++)
	{
		nComp = 0;
		for ( j= 0 ; j < componentdetails.size();j++)
		{
			for( k = 0; k < componentdetails[j].pinname.size();k++)
			{
				if (unsortednet[i].wirename.compare(componentdetails[j].pinname[k]) == 0) 
				{
					if(unsortednet[i].wireindex <=  componentdetails[j].busstartindex[k] )
					{
						if(unsortednet[i].wireindex >=  componentdetails[j].busendindex[k])
						{
							tempcomponentList.componentType.assign(componentdetails[j].componenttype);
							tempcomponentList.componentName.assign(componentdetails[j].componentname);
		...
		tempnetList.wireName.assign(unsortednet[i].wirename);
		netList.push_back(tempnetList);
		while (nComp--)
			tempnetList.componentList.pop_back();
	}
	writetofile(modulenamevector, componentdetails, unsortednet,netList, netlist_file);
}
...

{
	...
	{ //operations on tokenvector[i]
		...
		else if(tokenvector[i].compare("endmodule")==0)
		{
			...
			state = ENDMODULE;
			CheckComponentPinValidity(componentdetails,net);
		}
		else
		{
			state = COMPONENT;
...
int CheckComponent(struct componentstruct cs){
	...
	if((cs.componenttype == "and")||(cs.componenttype == "or")||(cs.componenttype == "xor")){
	...
	if((cs.componenttype == "evl_one")||(cs.componenttype == "evl_zero")||
...
void writetofile(const std::vector<std::string> & modulenamevector,
				 /*	const std::vector<struct wirestruct> & wiredetails,*/
				 const std::vector<struct componentstruct> & componentdetails,
				 const std::vector<struct netstruct> & net, 
				 const std::vector<struct netlist> & netList, 
				 ...
				 std::ofstream &netlist_file){
					 ...
					 for(mvs = 0; mvs<modulenamevector.size();mvs++){ //for every module in the code
						 netlist_file<< "module" << " "<< modulenamevector[mvs] << std::endl;

						 netlist_file<<"nets "<<net.size()<<std::endl;
						 for(n = 0; n< net.size();n++){
							 if(net[n].wireindex!=-1)
								 netlist_file<<"\t net "<< net[n].wirename<<"["<<net[n].wireindex<<"] " 
								 <<netList[n].componentList.size()<<std::endl;
							 else
								 netlist_file<<"\t net "<< net[n].wirename<<" "<<netList[n].componentList.size()<<std::endl;
							 for(m =0;m< netList[n].componentList.size();m++){
								 if(netList[n].componentList[m].componentName != "")
									 netlist_file<<"\t\t "<<netList[n].componentList[m].componentType<<" "
									 <<netList[n].componentList[m].componentName<<" "<<netList[n].componentList[m].pinLocation<<std::endl;
								 else
									 netlist_file<<"\t\t "<<netList[n].componentList[m].componentType<<" "
									 <<netList[n].componentList[m].pinLocation<<std::endl;

							 }
						 }

						 netlist_file<<"components "<<componentdetails.size()<<std::endl;
						 for(n = 0; n< componentdetails.size();n++){
							 if(componentdetails[n].componentname !="")
								 netlist_file<<"\t component "<< componentdetails[n].componenttype<<" "
								 << componentdetails[n].componentname<<" "
								 <<componentdetails[n].componentsize<<std::endl;
							 else
								 netlist_file<<"\t component "<< componentdetails[n].componenttype<<" "
								 <<componentdetails[n].componentsize<<std::endl;

							 for(m = 0; m< componentdetails[n].pinname.size();m++){
								 if(componentdetails[n].busstartindex[m] == -1)//single wire
									 netlist_file<<"\t\t pin "<<componentdetails[n].busstartindex[m]-componentdetails[n].busendindex[m]+1<<" "
									 <<componentdetails[n].pinname[m]<<std::endl;
								 else if(componentdetails[n].busstartindex[m] != componentdetails[n].busendindex[m]){// many wires from a bus
									 netlist_file<<"\t\t pin "<<componentdetails[n].busstartindex[m]-componentdetails[n].busendindex[m]+1;
									 //removed space after pin size in string above
									 for(k = componentdetails[n].busendindex[m]; k <= componentdetails[n].busstartindex[m]; k++){
										 netlist_file<<" ";//so that extra space is not added after last element
										 netlist_file<<componentdetails[n].pinname[m]<<"["<<k<<"]";
									 }
									 //end of for loop
									 ...
									 netlist_file<<"\t\t pin "<<componentdetails[n].busstartindex[m]-componentdetails[n].busendindex[m]+1<<" "
									 <<componentdetails[n].pinname[m]<<"["<<componentdetails[n].busstartindex[m]<<"]"<<std::endl;
							 }
...
//taken and heavily modified from https://en.wikibooks.org/wiki/Algorithm_Implementation/Search/Binary_search
{
...
int main(int argc, char *argv[])
{
	...
	std::vector<struct modulestruct> moduledetails;
	std::vector<struct componentstruct> componentdetails;
	std::vector<struct netstruct> net, unsortednet;
	std::vector<struct netlist> netList;
	std::vector<std::string> modulenamevector;
	...
	//strcpy(argv[1],"prj4Test1AND.evl");//counter_flat.evl");//"tris_lut.evl");//"s15850.evl");//"lfsr10.evl");
	...
	CreatenetList(componentdetails,unsortednet,modulenamevector,netList,netlist_file);
	...
	for ( j= 0 ; j < componentdetails.size();j++)
	{
		...
		if(componentdetails[j].componenttype == "evl_input")
		{
			input_file_name = firstargv+"."+ componentdetails[j].componentname+".evl_input";
			//std::ifstream readinput;
			readinput.open(input_file_name.c_str());
		}
		//	create the input file //Code only used for testing
/*		if(componentdetails[j].componenttype == "evl_input")
		{
			input_file_name = firstargv+"."+ componentdetails[j].componentname+".evl_input";
			std::ofstream evl_input_file(input_file_name.c_str());

			evl_input_file<<componentdetails[j].pinname.size();

			for(k = 0; k< componentdetails[j].pinname.size();k++){
				int pinsize = componentdetails[j].busstartindex[k] - componentdetails[j].busendindex[k] +1;
			...
			{
				apb = CreateInputFile(componentdetails ,j ,i+1);
				evl_input_file<<apb<<std::endl;
			}
		}*/
		firstsimulation.push_back(1);
	}
	for ( j= 0 ; j < NO_OF_SIMULATIONS;j++)
	{
		for (k = 0; k < componentdetails.size(); k++)
			componentdetails[k].simulationID = 0;

		//clear the vector
		temptv_vector.wirename = "";
		temptv_vector.wirevalueB = 0;
		temptv_vector.wirevalueZ = 0;


		for(k = 0;k < componentdetails.size();k++)
		{
			Simulate(argv[1],componentdetails,k,unsortednet,netList,j+1,0);
		}
		for(k = 0;k < componentdetails.size();k++)
		{
			Simulate(argv[1],componentdetails,k,unsortednet,netList,j+1,1);// generate evl_output
		}
...
{
	unsigned int i=0,j=0,k=0;//general purpose variables for loop
	...
	int retValue;//bonus
	//std::vector<struct componentstruct> tempcd;
	//struct componentstruct tempcditem;
...
//for evl input bonus1
	...
	{
		if((cd[i].componenttype == "not") && (evloutput == 0))//Initial Release 
		{
		...
		else if((cd[i].componenttype == "buf") && (evloutput == 0)) // to do Z implementation
		{
			if(cd[i].simulationID == TransId) return;
		...
		else if((cd[i].componenttype == "and") && (evloutput == 0))
		{
			...
			for(j = 1; j < cd[i].componentsize; j++)
			{
				...
				// find the input pin in the netList structure and correspond ComponentID to check if it is already computed
				CheckComponentExecStatus(cd[i],j,net,nl,&nlIndex,&clIndex);
				...
				{
					//	std::cout<<"CONTENTS OF currentValue  ="<<currentValue[k].wirename<<std::endl;
					if(currentValue[k].wirename.compare(cd[i].pinname[j]) == 0) 
					{
						found = 1;
						break;
					}
		...
		else if((cd[i].componenttype == "xor") && (evloutput == 0))
		{
			if(cd[i].simulationID == TransId) return;
		...
		else if((cd[i].componenttype == "evl_zero") && (evloutput == 0))
		{
			...
			for(k = 0; k < cd[i].componentsize; k++)
			{
				...
				{
					if(currentValue[j].wirename.compare(cd[i].pinname[k]) == 0) 
					{ 
						found = 1;
						break;
					}
				...
				if(found)// get the pin in the bus
				{
					if(cd[i].busstartindex[k] == -1)
						tempcv.wirevalue = currentValue[j].wirevalue[0] | 0x0;
					else 
					{
						tempcv.wirevalue =  UpdateCurrentValue(currentValue[j].wirevalue[0], result, lsb, msb);
					}
				}
				else 
			...
			for(k = 0; k< currentValue.size(); k++)
			std::cout<<"CONTENTS OF currentValue in evl_zero ="<<currentValue[k].wirename<<" "<<currentValue[k].wirevalue[0]<<std::endl;

			//std::cout<<"Value  OF currentValue[1] in evl_zero ="<<currentValue[1].wirevalue[0]<<std::endl;
		...
		else if((cd[i].componenttype == "evl_one") && (evloutput == 0))
		{
			...
			for(k = 0; k < cd[i].componentsize; k++)
			{
				...
				{
					if(cd[i].busstartindex[k] == -1)
						tempcv.wirevalue = currentValue[j].wirevalue[0] | 0x1;
					else 
					{
						tempcv.wirevalue =  UpdateCurrentValue(currentValue[j].wirevalue[0], result, lsb, msb);
					}
				...
				for(k = 0; k< currentValue.size(); k++)
				std::cout<<"CONTENTS OF currentValue in evl_one before insert ="<<currentValue[k].wirename<<" "<<currentValue[k].wirevalue[0]<<std::endl;
			...
			for(k = 0; k< currentValue.size(); k++)
			std::cout<<"CONTENTS OF currentValue in evl_one ="<<currentValue[k].wirename<<" "<<currentValue[k].wirevalue[0]<<std::endl;
			//std::cout<<"Value  OF currentValue[1] in evl_one ="<<currentValue[1].wirevalue[0]<<std::endl;

		}
		else if((cd[i].componenttype == "evl_dff") && (evloutput == 0))//Initial Release // //bonus
		{	
			if(cd[i].simulationID == TransId) return;
...
// if this D Flip flop is called first time , set its OUTPUT  to 0 as initial state of each output is supposed to be 0 as per project documnet
			if(firstsimulation[i]==1)
			{
				...
				for(k = 0; k< currentValue.size(); k++)
				{
					if(currentValue[k].wirename.compare(cd[i].pinname[0]) == 0) //pin 0 = Q
					{
						found = 1;
						break;
					}
				...
				if(found)
					tempcv.wirevalue = UpdateCurrentValue(currentValue[k].wirevalue[0], result, cd[i].busendindex[0], cd[i].busstartindex[0]);
				else 
					tempcv.wirevalue = UpdateCurrentValue(0, result, cd[i].busendindex[0], cd[i].busstartindex[0]);

				if(found)// earse the exsting value
					currentValue.erase(currentValue.begin()+k);	
				...
				for(j = 0, found = 0; j < prevValue.size(); j++)
				{
					if(prevValue[j].wirename.compare(currentValue[k].wirename) == 0) //pin 0 = Q
					{
						found = 1;
						break;
					}
...
//////////////////////////////////////////////////////////////////
				...
				if(found)
					tempcv.wirevalue = UpdateCurrentValue(prevValue[j].wirevalue[0], result, cd[i].busendindex[0], cd[i].busstartindex[0]);
				else
					tempcv.wirevalue = UpdateCurrentValue(0, result, cd[i].busendindex[0], cd[i].busstartindex[0]);
				...
				if (found)// erase the existing value
					prevValue.erase(prevValue.begin()+j);
/////////////////////////////////////////////////////////////////
				prevValue.insert(prevValue.begin()+j,tempcv_vector);
...
//copy CV = prevValue.
				...
				result = prevValue[j].wirevalue[0];
				...
				if(lsb != -1)// not a single wire
					result = result<<lsb; // move the bit to the location pointed by lsb  or msb of output pin; both are same for dff gate
						unsigned uu = currentValue[k].wirevalue[0];
				if(found)
					tempcv.wirevalue =  UpdateCurrentValue(currentValue[k].wirevalue[0], result, cd[i].busendindex[0], cd[i].busstartindex[0]);
				else
					tempcv.wirevalue =  UpdateCurrentValue(0, result, cd[i].busendindex[0], cd[i].busstartindex[0]);
				...
				//////////////////////////////////////
				tempcv.wirename.assign(prevValue[j].wirename);
			...
			for(k = 0; k< currentValue.size(); k++)//find input pin D value in currentValue vector
			{
				//std::cout<<"CONTENTS OF currentValue  ="<<currentValue[k].wirename<<std::endl;
			...
			prevValue.insert(prevValue.begin()+j,tempcv_vector);
			//std::cout<<"CONTENTS OF currentValue in evl_dff ="<<prevValue[0].wirename<<"["<<cd[i].busstartindex[0]<<"]"<<" "<<prevValue[0].wirevalue[0]<<std::endl;

			tempcv_vector.wirevalue.pop_back();

			for(k = 0; k< prevValue.size(); k++)
				std::cout<<"CONTENTS OF prevValue in evl_dff  end ="<<prevValue[k].wirename<<" "<<prevValue[k].wirevalue[0]<<std::endl;

		}
		else if((cd[i].componenttype == "evl_output") && (evloutput == 1))
		{
			if(cd[i].simulationID == TransId) return;
			...
			if(firstsimulation[i]==1)
			{
				...
				//std::cout<<"tempglobalcounter ="<<tempglobalcounter<<std::endl;
				int size = trisValue.size();
				//std::cout<<"trisValue.wirevalueZ.size() in first simulation ="<<yy<<std::endl;

				retValue = ComputeTrisValue();//bonus
				if (size)
					trisValue.clear();

				if(retValue == -1) return;// bonus
			...
			{
				std::cout<<"going into subsequent simulation of evl_output"<<std::endl;				output_file_name = argv+"."+ cd[i].componentname+".evl_output";
				...
				//std::cout<<"tempglobalcounter ="<<tempglobalcounter<<std::endl;
				//std::cout<<"trisValue.wirevalueZ.size() in subsequent simulation ="<<yyy<<std::endl;
				retValue = ComputeTrisValue();//bonus

				if (size)
					trisValue.clear();

				//std::cout<<"trisValue.wirevalueZ.size() in subsequent simulation after pop ="<<trisValue.wirevalueZ.size()<<std::endl;
		...
		else if((cd[i].componenttype == "evl_input") && (evloutput == 0))
		{
			...
			if(firstsimulation[i]==1)
			{
				...
				no_of_pins = (unsigned) atoi(filenum.c_str());//Nov
				//std::cout<<"\nfile input "<<filenum;
				...
				for (k= 0; k< no_of_pins; k++)
				{
					...
					else
					{
						...
						if (found)
						{
							...
							tempcv.wirevalue = UpdateCurrentValue(currentValue[j].wirevalue[0], pindata_read[k],
			...
			for(k = 0; k< currentValue.size(); k++) 
				std::cout<<"CONTENTS OF currentValue in evl_input ="<<currentValue[k].wirename<<" "<<currentValue[k].wirevalue[0]<<std::endl;

		}
		else if((cd[i].componenttype == "evl_lut") && (evloutput == 0))//bonus
		{
			std::string filenum;
			//std::vector<int> pinwidth_read;
			//std::vector<unsigned int> pindata_read;
...
//				std::cout<<"\nROM CREATED "<<tempcd.busstartindex[0]<<std::endl;
			...
			//Now tempcd has second argument stored as  a[4],a[5],a[6],a[7] for   evl_lut rom(mux_in, a[7:4]);
			//std::cout<<"Value  OF currentValue[1] in LUT  ="<<currentValue[1].wirevalue[0]<<std::endl;

			//check if a[4],a[5],a[6],a[7] have been computed in this cycle
			for(j = 0; j < tempcd.pinname.size() ;j++)
			{
				CheckComponentExecStatus(tempcd,j,net,nl,&nlIndex,&clIndex);
			...
			for(k = 0; k< currentValue.size(); k++)//find input pin value in currentValue vector
			{
				//std::cout<<"Name OF currentValue  ="<<currentValue[k].wirename<<std::endl;
				if(currentValue[k].wirename.compare(cd[i].pinname[1]) == 0)
				{
					found = 1;
					break;
			...
			std::ifstream readLUTinput;
			readLUTinput.open(input_file_name.c_str());
			if (!readLUTinput)
			...
			else
			{
				...
				readLUTinput>>filenum;// read word_width
				unsigned int word_width = atoi(filenum.c_str());

				readLUTinput>>filenum;// read address_width
				...
				//for(k = 0; k < data_to_read;k++)// read actual data
				{
					k = 0;
					do{
						readLUTinput>>filenum;// read pin data
						if(filenum == "")
							break;
						k++;
					}while(result >= k);
					...
					for(j = 0; j< currentValue.size(); j++)
					{
						if(currentValue[j].wirename.compare(cd[i].pinname[0]) == 0) 
						{
							found = 1;
							break;
						}
					...
					else
					{
						...
						if (found)
							tempcv.wirevalue = UpdateCurrentValue(currentValue[j].wirevalue[0], tempvar,
					...
					currentValue.insert(currentValue.begin()+j,tempcv_vector);
					tempcv_vector.wirevalue.pop_back();//monkey

				}
				for(k = 0; k < currentValue.size();k++)// clear the vector
				{
//					std::cout<<"\nROM LUT values "<<tempcv_vector.wirevalue[k]<<std::endl;
					//std::cout<<"\ncurrentValue in LUT "<<currentValue[k].wirename<<std::endl;
//					tempcv_vector.wirevalue.pop_back();
				}
		...
		else if((cd[i].componenttype == "tris") && (evloutput == 0))
		{
			...
			CheckComponentExecStatus(cd[i],2,net,nl,&nlIndex,&clIndex);
			...
			found = 0;lsb = 0; msb = 0;
			for(k = 0; k< currentValue.size(); k++)//find input pin en value in currentValue vector
			{
				//	std::cout<<"CONTENTS OF currentValue  ="<<currentValue[k].wirename<<std::endl;
				if(currentValue[k].wirename.compare(cd[i].pinname[2]) == 0) 
				{
					found = 1;
					break;
				}
			...
			if(found)// get the pin in the bus (input pin en)
			{
				if(cd[i].busstartindex[2] != -1)// not a single wire
				{
					...
					//std::cout<<"en pinvalue ="<<currentValue[k].wirevalue[0]<<std::endl;
					//std::cout<<"en pinname ="<<currentValue[k].wirename<<std::endl;

					en = (currentValue[k].wirevalue[0] >> lsb) & ~(~0 << (msb-lsb+1));
				}
				else 
					en = currentValue[k].wirevalue[0];
			}
			else
				en = 0;
			...
			if(found)// get the pin in the bus (input pin in)
			{
				if(cd[i].busstartindex[1] != -1)// not a single wire
				{
					msb = cd[i].busstartindex[1];
					lsb = cd[i].busendindex[1];
					in = (currentValue[k].wirevalue[0] >> lsb) & ~(~0 << (msb-lsb+1));
				}
				else 
					in = currentValue[k].wirevalue[0];
			}
			else
				in = 0;
			...
			{
				//if(found)
				//	tempcv.wirevalue =  out;//trisValue[k].wirevalueB|out;//UpdateCurrentValue(currentValue[k].wirevalue[0], out, cd[i].busendindex[0], cd[i].busstartindex[0]);
				//else
			...
			else
			{
				...
				//	tempcv.wirevalue = out;//trisValue[k].wirevalueZ|out;// UpdateCurrentValue(currentValue[k].wirevalue[1], out, cd[i].busendindex[0], cd[i].busstartindex[0]);
				//else
					tempcv.wirevalue = out;//0|out;// UpdateCurrentValue(0, out, cd[i].busendindex[0], cd[i].busstartindex[0]);
			}
			...
			if(en)
			{
				...
				trisvalueadd(temptv_vector, trisValue);//monkey
			...
			for(k = 0,found = 0; k< currentValue.size(); k++)//find output pin D
			{
				//std::cout<<"CONTENTS OF currentValue MUX_IN ="<<currentValue[k].wirename<<std::endl;
				if(currentValue[k].wirename.compare(cd[i].pinname[0]) == 0)
				{
					found = 1;
					break;
...
std::string CreatePinValueString(std::vector<struct componentstruct> & cd, /*std::vector<struct currentvalue> cv,*/unsigned int index)
{
	...
	for(j = 0; j< cd[i].pinname.size();j++)
	{
		...
		for(k = 0; k< currentValue.size(); k++)
		{
			if(currentValue[k].wirename.compare(cd[i].pinname[j]) == 0) 
			{
				found = 1;
				lsb = cd[i].busendindex[j];
				if (lsb == -1) lsb = 0;//bonus
				break;
			}
		}

		if(found)
			tempvalue = currentValue[k].wirevalue[0];//cd[i].pinvalue[j];
		else 
			tempvalue = 0;

		//sprintf(buffer,"%x",tempvalue);//currentValue[k].wirevalue[0]);
		...
		if(pin_size==1)//
		{
			...
			sprintf(buffer,"%x",(tempvalue>>lsb) & 0x1);// bonus
	...
	//for(k = 0; k < currentValue.size();k++)
	//	std::cout<<"CONTENTS OF currentValue in evl_output ="<<currentValue[k].wirename<<std::endl;
...
unsigned int UpdateCurrentValue(unsigned OrgNumber, unsigned NewNumber, int endIndex, int startIndex)
...
void CheckComponentExecStatus(struct componentstruct cd,unsigned pinindex, const std::vector<struct netstruct> & net,
							  std::vector<struct netlist>&nl,unsigned *nlIndex,unsigned *clIndex)
{
	...
	for(k = 0; k< nl.size(); k++)//find input pin value in currentValue vector
	{
		...
		if(nl[k].wireName.compare(cd.pinname[pinindex]) == 0)
		{
